...
if __name__ == '__main__':
    table = pd.read_csv(interDir + fileName, index_col = "成交日期")
    #table.dropna(0)
    ...
    for oneDay in dates:
        ...
        for ite in range(ites):
            ...
            opt = value[3]
            spr = str.split(opt, ":")
            if spr[0] == "证券买入" or spr[0] == "证券卖出":
                dcode = dealCodes(code)
                if code == code:
                        ed_csv(path1, index_col = index) sopt = True
                    code = int(code)
                else:
                    continue
                dcode = dealCodes(code)
