...
if __name__ == '__main__':
    table = pd.read_csv(interDir + fileName, index_col = "发生日期")
    #table.dropna(0)
    ...
    for oneDay in dates:
        ...
        for ite in range(ites):
            ...
            opt = value[3]
            if code == code:
                if (opt == "证券卖出清算" or opt == "卖出" or opt == "证券买入清算" or opt == "买入"):
                    code = dealCodes(int(code))
                else:
                    continue
            else:
                continue
            ...
            #print dcode
            if (dcode != "600656S"):
                if opt == "证券卖出清算" or opt == "卖出":
                    vf.sell(dcode, dealAmount, dealPrice)
                elif opt == "证券买入清算" or opt == "买入":
        ...
        if date_count == 1:
            ...
            cur_returns_ratio = (vf.returns / (vf.origin + 0.001)) * 100.0
            ...
            cur_returns_ratio = (vf.returns / (today_captical + 0.001)) * 100.0
            vf.set_ratio(cur_returns_ratio)
            user_returns += (vf.total_assets - today_captical)
        vf.set_total_returns(user_returns)
        vf.set_total_ratio((user_returns / (vf.origin + 0.001)) * 100)
        pre_date = dDate
        #print vf
