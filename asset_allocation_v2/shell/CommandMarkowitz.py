...
def pos_update(markowitz, alloc, optappend, sdate, edate, optcpu):
    ...
    if algo == 1:
        ...
        trade_date = ATradeDate.trade_date(begin_date=sdate, lookback=lookback)
        assets = dict([(asset_id, Asset(asset_id)) for asset_id in list(assets.keys())])
        allocate = CppiAllocate2('ALC.000001', assets, trade_date, lookback, bound=bounds, period=22)
        df = allocate.allocate_cppi()
        ...
        trade_date = ATradeDate.trade_date(begin_date=sdate, lookback=lookback)
        assets = dict([(asset_id, Asset(asset_id)) for asset_id in list(assets.keys())])
        allocate = RpAllocate('ALC.000001', assets, trade_date, lookback, bound=bounds, period=1)
        df = allocate.allocate()
        ...
        trade_date = ATradeDate.trade_date(begin_date=sdate, lookback=lookback)
        assets = dict([(asset_id, Asset(asset_id)) for asset_id in list(assets.keys())])
        allocate = CppiAllocate3('ALC.000001', assets, trade_date, lookback, bound=bounds, period=22)
        df = allocate.allocate_cppi()
    ...
    #每四周做平滑
    no_rolling_algos = [1, 16, 17, 18, 19, 20, 21, 23]
