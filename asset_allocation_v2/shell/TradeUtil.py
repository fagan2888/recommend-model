...
def cost(all_code_position):
    ...
    for record in all_code_position:
        ...
        if float(record[3])>=0.1:
            ...
            po_fee = 0
        if str(record[0])+'--'+str(record[1]) in risk_position_buy_po_fee:
    ...
    for k,v in list(risk_position_other.items()):
        if v<1:
            if str(213009) in risk_position[k]:
                risk_position[k][str(213009)] += (1-v)
            else:
                risk_position[k][str(213009)] = 1-v

    risk_position_cost = {}
    for k,v in list(risk_day.items()):
    ...
    #            continue
            ...
            if laste == {}:
                ...
                fee = 0
                for j,l in list(risk_position[k+'--'+i].items()):
                    tmp_fee = DB.getFee(j,0,10000*l)
                    fee += tmp_fee
                    fund_type = DB.getFundType(j)
                    if fund_type in fee_dict:
            ...
            #        continue
                ...
                for code,share,time in zip(code_list,share_list,time_list):
                    if code in risk_position_date_holding[k+'--'+i]:
                ...
                all_fee += cg['fee']
                all_cost += cg['cost']
                for m,n in list(cg['fee_dict'].items()):
                    if m in fee_dict:
...
def change(code_list_origin,share_list_origin,time_list_origin,position_origin,day):
    ...
    for k,v in list(del_index_list.items()):
        share_list[k] -= v
...
def getDailyNav(position):
    ...
    all_result = []
    for k,v in list(risk_position_date.items()):
        ...
        for i in range((end - begin).days+1):
            ...
            if len(tmp_v)>0 and isDay(day,tmp_v[0]) == 0:
                for code,share in list(risk_position_date_holiding[str(k)+'--'+str(day)].items()):
                    amount = DB.getAmount(code,share,day)
                    amounts += amount
                ...
                for code,share in list(old_position.items()):
                    amount = DB.getAmount(code,share,day)
                    amounts += amount
                nav = amounts/10000
                inc = (nav-old_nav)/old_nav
