...
class StockPortfolio: # (metaclass=MetaClassPropertyDecorator)
    ...
    def portfolio_analysis(self, reindex=None):
        ...
        MDD_end_dt = np.argmax(np.maximum.accumulate(self.ser_portfolio_nav) - self.ser_portfolio_nav)
        MDD_start_dt = np.argmax(self.ser_portfolio_nav[:MDD_end_dt])
        
        MDD = (self.ser_portfolio_nav[MDD_start_dt] - self.ser_portfolio_nav[MDD_end_dt]) / self.ser_portfolio_nav[MDD_start_dt]
        
        sharpe_ratio = (portfolio_return*244/len(list(reindex)) - free_risk_rate)/ std_excess_return*(244/len(list(reindex)))**0.5
        
        return portfolio_return*244/len(list(reindex)), std_excess_return*(244**0.5), sharpe_ratio, MDD_start_dt, MDD_end_dt, MDD
...
# In[2]
#EG:
class StockPortfolio50MinimumVolatility(StockPortfolio):

    def __init__(self, index_id, reindex, look_back, *args, **kwargs):
    
        super(StockPortfolio50MinimumVolatility, self).__init__(index_id, reindex, look_back, *args, **kwargs)
...
#        set_trace()
    ...
    def _load_stock_pool(self, trade_date):
        ...
        stock_pool = super(StockPortfolio50MinimumVolatility, self)._load_stock_pool(trade_date)
    
        stock_ids_by_industry = self.df_stock_industry.loc[self.df_stock_industry.sw_level1_code=='340000'].index
        stock_pool = stock_pool.reindex(stock_ids_by_industry)
...
class StockPortfolioLowBeta(StockPortfolioMarketCap):

    def __init__(self, index_id, reindex, look_back, percentage, *args, **kwargs):

        super(StockPortfolioLowBeta, self).__init__(index_id, reindex, look_back, percentage, *args, **kwargs)
        
#        self.reindex_total[:-1] = ATradeDate.trade_date(begin_date=reindex[0], end_date=reindex[-1], lookback=look_back+1)
        
        tmp_portfolio = StockPortfolioMarketCap(index_id, self.reindex_total[:-1], 1, *args, **kwargs)
        tmp_pos = tmp_portfolio.calc_stock_pos_days()
#        set_trace()
        self.indexing_ret = pd.DataFrame((tmp_pos.values * tmp_portfolio.df_stock_ret.values), index=tmp_portfolio.df_stock_ret.index).sum(axis=1)
        self._percentage = percentage
    ...
    def _calc_stock_pos(self, trade_date):
        ...
        stock_ids = df_stock_ret.columns
        df_stock_status = self.df_stock_status.loc[df_stock_ret.index, stock_ids]
        indexing_ret = self.indexing_ret.loc[df_stock_ret.index]
        indexing_ret_std = indexing_ret.std()
        df_stock_ret[df_stock_status>2] = np.nan
        df_stock_ret = df_stock_ret.loc[df_stock_ret.index.isin(indexing_ret.index)]
        
        df_stocks_retANDindexing_ret = pd.merge(pd.DataFrame(df_stock_ret), pd.DataFrame(indexing_ret), left_index=True, right_index=True, how='inner')
        
        df_stocks_retANDindexing_ret_cov = df_stocks_retANDindexing_ret.cov()
        Beta = df_stocks_retANDindexing_ret_cov.iloc[:-1,-1]/(indexing_ret.std()**2)
#        .apply(lambda x: 1.0/x if x>0.0 else 0.0)
        
        #Low beta condition quantile as self.percentage
        Low_Beta = Beta.sort_values(ascending=True).iloc[:int(self.percentage*stock_ids.shape[0])]
        
        df_stock_ret_low_beta = df_stock_ret[Low_Beta.index]
        #Low volatility condition quantile as .50
        df_stock_ret_low_beta_low_vol_ = df_stock_ret_low_beta.std().sort_values().iloc[:(int(0.5*df_stock_ret_low_beta.shape[1]))]
        df_stock_ret_low_beta_low_vol = df_stock_ret[df_stock_ret_low_beta_low_vol_.index]
        
        
        stock_pos = pd.Series(0.0, index=stock_ids, name=trade_date)
        #Equally weighted
        try:
            stock_pos.loc[df_stock_ret_low_beta_low_vol.columns] = 1.0 / (int(0.5*df_stock_ret_low_beta.shape[1]))
        except Exception:
            set_trace()
...
class StockPortfolioSectorNeutralLowBeta(StockPortfolioMarketCap):
...
#        set_trace()
    ...
    def _calc_stock_pos(self, trade_date):
        ...
        stock_ids = df_stock_ret.columns
        df_stock_status = self.df_stock_status.loc[df_stock_ret.index, stock_ids]
        indexing_ret = self.indexing_ret
        indexing_ret_std = indexing_ret.std()

        df_stock_ret[df_stock_status>2] = np.nan
        df_stock_ret = df_stock_ret[df_stock_ret.index.isin(indexing_ret.index)]
...
if __name__ == '__main__':
    ...
    dict_portfolio['StockPortfolioLowBeta'] = StockPortfolioLowBeta(index_id, trade_dates, look_back, percentage)
    df_portfolio_nav['StockPortfolioLowBeta'] = dict_portfolio['StockPortfolioLowBeta'].calc_portfolio_nav()
