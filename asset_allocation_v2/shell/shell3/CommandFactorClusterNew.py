...
def fc_rolling(ctx, optid):
    ...
    for date in trade_dates:
        ...
        for k, v in list(fc.asset_cluster.items()):
...
class FactorCluster(object):
    ...
    def __init__(self, assets, n_clusters, start_date, end_date, fdate, method = 'cor', bf_ids = None):
    ...
    def train(self, ret, n_clusters, method = 'corr'):
        ...
        if n_clusters >= 2:
            ...
            silhouette_samples_value = silhouette_samples(distance_matrix(ret.T.values), cluster.labels_, 'precomputed')
            self.silhouette_samples_value = silhouette_samples_value.mean()/silhouette_samples_value.std()
        else:
    ...
    def score(self):
        ...
        inner_score = []
        all_nav_ret = []
        for layer in list(self.asset_cluster.keys()):
