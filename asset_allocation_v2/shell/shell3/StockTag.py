...
from . import FundIndicator as fi
...
def largecapfitness(funddf, indexdf, ratio):
    ...
    fitness = {}
    for code in list(largecaptag.keys()):
        rs = largecaptag[code]
        fitness[code] = (np.mean(rs), np.std(rs), (np.mean(rs) - Const.rf) / np.std(rs))


    x = fitness
    sorted_x = sorted(iter(list(x.items())), key=lambda x : x[1][2], reverse=True)
...
def smallcapfitness(funddf, indexdf, ratio):
    ...
    fitness = {}
    for code in list(smallcaptag.keys()):
        rs = smallcaptag[code]
        fitness[code] = (np.mean(rs), np.std(rs), (np.mean(rs) - Const.rf) / np.std(rs))
...
def declinefitness(funddf, indexdf, ratio):
    ...
    fitness = {}
    for code in list(declinetag.keys()):
        rs = declinetag[code]
        fitness[code] = (np.mean(rs), np.std(rs), (np.mean(rs) - Const.rf) / np.std(rs))
...
def oscillationfitness(funddf, indexdf, ratio):
    ...
    fitness = {}
    for code in list(oscillationtag.keys()):
...
def growthfitness(funddf, indexdf, ratio):
    ...
    fitness = {}
    for code in list(growthcaptag.keys()):
...
def valuefitness(funddf, indexdf, ratio):
    ...
    fitness = {}
    for code in list(valuecaptag.keys()):
        rs = valuecaptag[code]
        fitness[code] = (np.mean(rs), np.std(rs), (np.mean(rs) - Const.rf) / np.std(rs))
...
def positionprefer(funddf, ratio):
    ...
    x = positiontag
    sorted_x = sorted(iter(list(x.items())), key=lambda x : x[1], reverse=True)
