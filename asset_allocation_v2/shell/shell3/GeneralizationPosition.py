...
import pandas as pd
import string
from . import GeneralizationTrade
import MySQLdb
from . import config
from . import DFUtil;
...
def risk_position():
    ...
    for i in range(0, len(dates)):
        ...
        #for risk_rank in range(10, 11):
            ...
            total_weight = 0
            ws = {}
            for col in list(highriskposition.keys()):
            ...
            for col in list(lowriskposition.keys()):
...
def clean_same(re):
    ...
    result = []
    for i in re:
        if str(i[0])+'--'+str(i[1]) in day_list:
            result.append(i)
    return result
...
def is_same(tmp1,tmp2):
    flag = 0.03
    change = 0
    for k,v in list(tmp1.items()):
        if k in tmp2:
            change += abs(v-tmp2[k])
        else:
            change += v
    for k,v in list(tmp2.items()):
        if k in tmp1:
            pass
        else:
            change += v
