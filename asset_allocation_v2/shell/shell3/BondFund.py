...
class BondFundFilter(object):
    def __init__(self):
        ...
        # 基金经理
        self.manager_num = 5
        self.cur_manager_days = 365

        # 机构持有比例
        self.holding_ratio = 0.5
    ...
    def cal_sharpe(self):
        ...
        for sid in sids:
            ...
            anal_ratio = mean_ratio * 252.0
            return_std = np.std(one_ratios) * np.sqrt(252.0)
            sharpe_two = anal_ratio / return_std
            #print one_ratios
            #print mean_ratio
            ...
            anal_ratio = mean_ratio * 252.0
            return_std = np.std(one_ratios) * np.sqrt(252.0)
            sharpe_five = anal_ratio / return_std
    ...
    def filter_types(self):
        ...
        按基金类型过滤
        """
        BondFundPool.load_bond_funds(self.types)
