...
def save(gid, criteria_id, df):
    ...
    #
    db = database.connection('asset')
    t2 = Table('on_online_criteria', MetaData(bind=db), autoload=True)
    columns = [literal_column(c) for c in (df.index.names + list(df.columns))]
    s = select(columns, (t2.c.on_online_id == gid)).where(t2.c.on_criteria_id == criteria_id)
    df_old = pd.read_sql(s, db, index_col=['on_online_id', 'on_criteria_id', 'on_date'], parse_dates=['on_date'])
    if not df_old.empty:
