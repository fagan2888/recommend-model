...
@click.option('--id', 'optid', help='specify investor id')
@click.option('--type', 'opttype', default='9', help='type type(8:with fee; 9:without fee')
...
@investor.command()
@click.option('--id', 'optid', help='ids of investor to update')
@click.option('--type', 'opttype', default='9', help='type type(8:with fee; 9:without fee')
...
def nav_update(alloc, xtype, debug, optfee, optt0, enddate):
    ...
    if not sr_contrib.empty:
        ...
        df_contrib.index.names=['is_date', 'is_return_type', 'is_fund_id']
