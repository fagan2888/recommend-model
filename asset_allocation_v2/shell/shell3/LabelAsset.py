...
from . import StockTag as ST
from . import Data
from numpy import *
import datetime
from . import FundFilter
from . import FundIndicator
from . import FundSelector
from . import Const
import pandas as pd
import time
...
def stockLabelAsset(allocationdata, dates, his_week, interval):
    ...
    print((list(range(his_week, len(dates)))))
...
def bondLabelAsset(allocationdata, dates, his_week, interval):
    ...
    pre_ratebond        = ['217003']
    pre_creditbond      = ['217003']
    pre_convertiblebond = ['217003']
    ...
    for i in range(his_week, len(dates)):
        ...
        if (i - his_week) % interval == 0:
            ...
            if 'ratebond' not in tag or len(tag['ratebond']) == 0:
                tag['ratebond'] = pre_ratebond
            else:
                pre_ratebond    = tag['ratebond']
            if 'creditbond' not in tag or len(tag['creditbond']) == 0:
                tag['creditbond'] = pre_creditbond
            else:
                pre_creditbond  = tag['creditbond']
            if 'convertiblebond' not in tag or len(tag['convertiblebond']) == 0:
