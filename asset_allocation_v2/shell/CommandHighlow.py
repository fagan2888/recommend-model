...
@click.option('--limit/--no-limit', 'optlimit', default=False, help='calculate time varying limit')
@click.option('--id', 'optid', help='specify markowitz id')
@click.option('--name', 'optname', default=None, help='specify highlow name')
...
@click.option('--high', 'opthigh', type=int, default=0, help='high asset id')
@click.option('--low', 'optlow', type=int, default=0, help='low asset id')
@click.option('--riskmgr', 'optriskmgr', default='*', help='with riskmgr')
@click.option('--riskmgr/--no-riskmgr', 'optriskmgr', default=True, help='with riskmgr or not')
@click.option('--risk', 'optrisk', default='10,1,2,3,4,5,6,7,8,9', help='which risk to calc, [1-10]')
@click.option('--end-date', 'optenddate', default=None, help='calc end date for nav')
@click.option('--start-date', 'optstartdate', default='2012-07-06', help='start date')
@click.pass_context
...
def allocate(ctx, optid, optname, opttype, optreplace, opthigh, optlow, optriskmgr, optrisk):
    ...
    if optid is not None:
        ...
        if not (isinstance(max_id, int) or isinstance(max_id, int)):
    ...
    if optname is None:
        optname = '高低风险%s' % today.strftime("%m%d")
    #
    # 加载用到的资产
    #
    ...
    if '11310100' not in df_asset.index:
        df_asset.loc['11310100'] = (0, '货币(低)', 31, 0, '11310100')
    if '11310101' not in df_asset.index:
        df_asset.loc['11310101'] = (0, '货币(高)', 31, 0, '11310101')
...
def cal_limit(highlow):
    ...
    start_date = high_nav.index[0] if high_nav.index[0] > low_nav.index[0] else low_nav.index[0]

    trade_dates = DBData.trade_date_index(start_date = '2009-01-01')
    # test_dates = DBData.trade_date_index(start_date = '2012-07-06')
    test_dates = DBData.trade_date_index(start_date = start_date)[lookback:]
...
def jiao2(highlow, alloc, ratio_hl):
    ...
    if not df_high.empty:
        df_high = df_high.reindex(index, method='pad')
        df_high_riskmgr = df_high_riskmgr.reindex(index, method='pad')
        ratio_h = ratio_hl.reindex(index).fillna(method = 'ffill').fillna((risk - 0.1) / 0.9).ratio_h.rolling(4, min_periods = 1).mean()
    ...
    if not df_low.empty:
        df_low = df_low.reindex(index, method='pad')
        df_low_riskmgr = df_low_riskmgr.reindex(index, method='pad')
        ratio_l = ratio_hl.reindex(index).fillna(method = 'ffill').fillna(1.0 - (risk - 0.1) / 0.9).ratio_l.rolling(4, min_periods = 1).mean()
...
def yao(highlow, alloc):
    ...
    if risk > 1.0 and high.startswith('MZ.00007'):
        df_high_riskmgr.loc['2019-02-01', 'ERI000001'] = 1.0
        df_high_riskmgr.loc['2019-02-01', 'ERI000002'] = 1.0
        df_high_riskmgr.loc['2019-02-01', '120000053'] = 1.0
        df_high_riskmgr.loc['2019-02-01', '120000056'] = 1.0
        df_high_riskmgr.loc['2019-02-01', '120000058'] = 1.0
        df_high_riskmgr.loc['2019-02-01', '120000073'] = 1.0
        df_high_riskmgr.loc['2019-02-01', '120000081'] = 1.0

        df_high_riskmgr.loc['2019-02-01', 'MZ.FA0010'] = 1.0
        df_high_riskmgr.loc['2019-02-01', 'MZ.FA0050'] = 1.0
        df_high_riskmgr.loc['2019-02-01', 'MZ.FA0070'] = 1.0
    ...
    if high.startswith('MZ.00007'):
        if risk == 10.0:
            overflow = (0.0478 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0254 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0478
            df_h.loc['2019-02-01', '120000081'] = 0.0254
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 9.0:
            overflow = (0.0431 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0224 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0431
            df_h.loc['2019-02-01', '120000081'] = 0.0224
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 8.0:
            overflow = (0.0397 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0201 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0397
            df_h.loc['2019-02-01', '120000081'] = 0.0201
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 7.0:
            overflow = (0.0353 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0174 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0353
            df_h.loc['2019-02-01', '120000081'] = 0.0174
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 6.0:
            overflow = (0.0307 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0139 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0307
            df_h.loc['2019-02-01', '120000081'] = 0.0139
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 5.0:
            overflow = (0.0260 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0124 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0260
            df_h.loc['2019-02-01', '120000081'] = 0.0124
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 4.0:
            overflow = (0.0207 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0098 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0207
            df_h.loc['2019-02-01', '120000081'] = 0.0098
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 3.0:
            overflow = (0.0159 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0072 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0159
            df_h.loc['2019-02-01', '120000081'] = 0.0072
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
        elif risk == 2.0:
            overflow = (0.0104 - df_h.loc['2019-02-01', 'ERI000001'] ) + (0.0045 - df_h.loc['2019-02-01', '120000081'])
            df_h.loc['2019-02-01', 'ERI000001'] = 0.0104
            df_h.loc['2019-02-01', '120000081'] = 0.0045
            df_h.loc['2019-02-01', '120000014'] = df_h.loc['2019-02-01', '120000014'] - overflow  / 2
            df_h.loc['2019-02-01', 'MZ.FA1010'] = df_h.loc['2019-02-01', 'MZ.FA1010'] - overflow  / 2
...
@highlow.command()
@click.option('--src', 'optsrc', help='src id of highlow to copy from')
@click.option('--dst', 'optdst', help='dst id of highlow to copy to')
@click.option('--list/--no-list', 'optlist', default=False, help='list instance to update')
@click.pass_context
