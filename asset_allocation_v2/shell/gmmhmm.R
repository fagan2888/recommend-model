...
regime_gmmhmm <- function(price_data, target_index = 1, nstate = 0) {
  ...
  ret = rollapply(prices, 20, mean) / rollapply(prices, 120, mean) - 1
  #print(rollapply(prices, 120, mean, fill = NA))
...
hmm_training <- function(gmm, data_training, data_testing = NULL, ret_target) {
  ...
  state <- cbind(data_training[,0], hmm_fitted$yhat)
  #print(tail(hmm_fitted$yhat, 10))
  print(tail(state, 1000))
