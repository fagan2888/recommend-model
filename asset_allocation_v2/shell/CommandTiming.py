...
from TimingMaHmm import TimingMaHmm
...
def signal_update_ma_hmm(timing):
    '''calc timing signal for singe timing instance
    '''
    #
    # 加载OHLC数据
    ...
    #df_new = TimingMaHmm(ori_data = df_nav, timing = timing, trade_dates = trade_dates, start_date = df_nav.index[252 * 5] if df_nav.index[252 * 5] > datetime(2012,5,1) else '2012-05-01').timing()
    df_new = TimingMaHmm(ori_data = df_nav, timing = timing, trade_dates = trade_dates, start_date = '2006-01-01').timing()
    df_new = df_new[['tc_signal']]
