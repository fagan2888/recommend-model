...
@train.command()
@click.pass_context
def user_redeem_xgboost(ctx):

    ratio = 10
    files = os.listdir('feature_data')
    datas = []
    for f in files:
        if f.startswith('redeem'):
            data = np.load('feature_data/' + str(f))
            data_positive = data[data[:,-1] == 1.0] 
            data_negative = data[data[:,-1] == 0.0] 
            resample_data_negative = resample(data_negative, n_samples=len(data_positive) * ratio)
            resample_data = list(np.vstack([data_positive, data_negative]))
            random.shuffle(resample_data)
            ...
            dtrain = xgb.DMatrix(X_train, label=y_train, nthread = 32)
            dval = xgb.DMatrix(X_val, label=y_val, nthread = 32)
            evallist = [(dtrain,'train'),(dval, 'val')]
            num_round = 10000
            #bst = xgb.train(param, dtrain, num_round, evallist, early_stopping_rounds = 50)
            bst = xgb.train(param, dtrain, num_round, evallist, early_stopping_rounds = 50, verbose_eval = 50)

            bst.save_model('model/' + str(f) + '.xgb')


@train.command()
@click.pass_context
def user_buy_xgboost(ctx):
...
@train.command()
@click.pass_context
def user_buy_xgboost_predict(ctx):
    ...
    for f in files:
        if f.startswith('buy'):
            ...
            bst.load_model('model/' + str(f))


            yesterday = (datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d')
            df = pd.read_csv('predict_feature_data/buy_predict_feature_' + yesterday + '.csv', index_col = ['uid', 'date'])
            uids = df.index.get_level_values(0)
            data = np.array(df.values)
            
            dtrain = xgb.DMatrix(data[:,0:-1], nthread = 24)

            y_preds.append(bst.predict(dtrain))

    y_pred = np.array(y_preds).mean(axis = 0)
    y_pred_df = pd.DataFrame(y_pred, index = uids, columns = ['pred_v'])

    y_pred_df = y_pred_df.sort_values('pred_v',ascending = False)
    print(y_pred_df)
    yesterday = (datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d')
    y_pred_df.to_csv('predict_user/buy_' + yesterday + '.csv')


@train.command()
@click.pass_context
def user_redeem_xgboost_predict(ctx):
    ...
    for f in files:
        if f.startswith('redeem'):
            ...
            yesterday = (datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d')
            df = pd.read_csv('predict_feature_data/redeem_predict_feature_' + yesterday + '.csv', index_col = ['uid', 'date'])
            uids = df.index.get_level_values(0)
            data = np.array(df.values)
    ...
    print(y_pred_df)
    yesterday = (datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d')
    y_pred_df.to_csv('predict_user/redeem_' + yesterday + '.csv')
