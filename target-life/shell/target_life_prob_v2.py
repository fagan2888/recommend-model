...
if __name__ == '__main__':
    ...
    num = 0
    dates = dfr.index
    for i in range(0, len(dates) - months):

        v = 1.0
        tmp_target_r = target_r
        d = dates[i]
        ...
        for j in range(0, months):
            ...
            n = months - j
            x = 0
            res = scipy.optimize.minimize(obj_fun, x, (tmp_target_r, n), method='SLSQP')
            x = res.x[0]
            risk_level = -1
            for p in range(0, len(rs)):
                if (p == 0 and x < rs[p]) or (x >= rs[p - 1] and x <= rs[p]):
                    risk_level = p + 1
                    break

            #print d, tmp_d, risk_level, v

            if risk_level == -1:
                #print d, tmp_d, v, 'not arrival'
                #break
                risk_level = 10
