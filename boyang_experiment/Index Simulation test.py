...
Timeseries_MC=[Gaussian_Copula(Index_data.iloc[:Windows_size+i,:],0.1,1e+5,0.99)for i in range(Windows_length)]
...
Timeseries_VaR_Summary=pd.DataFrame(data=Timeseries_VaR_Summary,columns=Copula.columns,index=Index_data.index[Windows_size:Windows_size+Windows_length])

Index_backtesting=Index_data_Chg[Index_data_Chg.index.isin(Timeseries_VaR_Summary.index)]
Index_backtesting_Indicator=Index_backtesting<=Timeseries_VaR_Summary
Index_backtesting_Indicator_1D=pd.DataFrame(data=Index_backtesting_Indicator.values,columns=Index_backtesting_Indicator.columns,index=Index_backtesting_Indicator.index+timedelta(days=1))

Index_data_Chg_Triggered=Index_data_Chg[Index_backtesting_Indicator_1D].fillna(value=0)
Index_data_Chg_Triggered1=Index_data_Chg_Triggered.cumsum()

Index_data_Chg_Triggered.to_csv('Index_data_Chg_Triggered.csv')
Index_data_Chg_Triggered1.to_csv('12.csv')
