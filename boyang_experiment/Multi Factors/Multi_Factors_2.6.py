...
Numerical Factors
'''
Beta=pd.DataFrame(data=np.asarray([np.cov(Ret_indexing.values.ravel()[:i],Return.values[:i,j])[0,1]/np.std(Ret_indexing.values[:i]) for i in range(Return.shape[0]) for j in range(Return.shape[1])]).reshape(Return.shape),index=Return.index,columns=Return.columns)

Month12std=Return.rolling(252).std().dropna()
#Idiosyncratic volatility
Idio_variance=Return.expanding(1).var(ddof=1)-np.repeat(Ret_indexing.expanding(1).var(ddof=1).values,Return.shape[1]).reshape(Return.shape)
Idio_std=Return.expanding(1).std(ddof=1)-np.repeat(Ret_indexing.expanding(1).std(ddof=1).values,Return.shape[1]).reshape(Return.shape)
...
#
#Single_factor_retrace(1/Month12std,Return,Ret_indexing)
...
#Single_factor_retrace(PB_ratio,Return,Ret_indexing)
#Single_factor_retrace(1/Beta,Return,Ret_indexing)
Single_factor_retrace(Idio_variance,Return,Ret_indexing)
Single_factor_retrace(Idio_std,Return,Ret_indexing)

#
#GLS_compare_summary(Ret_indexing,Weighted_by_factors(Month12std).mean(axis=1))

GLS_compare_summary(Ret_indexing,Beta.dropna().mean(axis=1))
...
GLS_compare_summary(Ret_indexing,Idio_variance.dropna().mean(axis=1))
GLS_compare_summary(Ret_indexing,Idio_std.dropna().mean(axis=1))


Single_factor_retrace(1/Beta,Return,Ret_indexing)

GLS_compare_summary(Ret_indexing,Momentum(Return,12,3).mean(axis=1))
GLS_compare_summary(Ret_indexing,PS_ratio.mean(axis=1))
