...
A=Momentum(Return,2,0.5).mean(axis=1).cumsum().plot()
...
A=MomentumR1.rank(ascending=False,axis=1).applymap(lambda x: 1/6 if x<=6 else (-1/6 if x>=12 else 0.0001))


Aa=A.index.isin([A.index[0+i*30] for i in range(A.shape[0]//30)])*1
...
AAAA=AAAA.applymap(lambda x: float('Nan') if x==0 else x)
AAAA=AAAA.fillna(method='ffill').applymap(lambda x: 0 if x ==0.0001 else x)
AAAAQ=AAAA*Return
AAAAQ=AAAAQ.dropna()
AAAAQ.mean(axis=1).cumsum().plot()
