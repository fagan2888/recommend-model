...
def Auto_Corr_Normality_multi_tests(residuals_regression,regressor):
    ...
    #Homoskedasticity
    C1=sm.stats.diagnostic.het_breuschpagan(residuals_regression,regressor)[2]
    C2=sm.stats.diagnostic.het_goldfeldquandt(residuals_regression,regressor)[2]
...
def GLS_compare_summary(Y,X):
    ...
    OLS_resid=sm.regression.linear_model.OLS(Y,X).fit().resid
...
#    Equally weighted L-S strategy at 30% rank quantile of portfolio 
    ...
    Weight=Momentum1.rank(ascending=False,axis=1).applymap(lambda x: 1/Nb_stock if x<=Nb_stock else (-1/Nb_stock if x>=stock_return.shape[1]-Nb_stock else 0))
    
    Index=[Weight.index[0+i*H] for i in range(Weight.shape[0]//H)]
    Weight.shift(-H)
...
#Single_factor_retrace(PE_ratio,Return,Ret_indexing).cumsum()
#Single_factor_retrace(np.log(stock_mkt_cap),Return,Ret_indexing)
#Single_factor_retrace(MomentumR12H3,Return,Ret_indexing)
#Single_factor_retrace(PB_ratio,Return,Ret_indexing)
#
#
...
A=Weighted_by_factors(MomentumR1)
int(A.sum(axis=1).sum())==A.shape[0]
 

A=MomentumR1.iloc[0:1,:].rank(axis=1)
A=MomentumR1.rank(ascending=False,axis=1).applymap(lambda x: 1 if x<=6 else (-1 if x>=12 else 0))


Aa=A.index.isin([A.index[0+i*30] for i in range(A.shape[0]//30)])
#Aa=[A.index[0+i*30] for i in range(A.shape[0]//30)]



AAA=pd.DataFrame(data=np.repeat(list(Aa),A.shape[1]).reshape(len(Aa),A.shape[1]),index=A.index,columns=A.columns)
AAAA=A*AAA
