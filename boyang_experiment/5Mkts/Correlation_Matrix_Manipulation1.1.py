...
columns_name = ['Trd_dt', '000001.SH', '39001.SZ','HSI.HI','SP500.SPI','NDX.GI']
...
for i in range(Ret.shape[0] - Ini_Windows_size):
    ...
    Cov_ret = Ret.iloc[i:Ini_Windows_size + i, :].corr()
...
#     plt.title('%s Corr para' % (Cov_ret_names[i]))
...
for i in range(Cov_ret_TS_Co_Inte.shape[1]):
    ...
    plt.title('%s prjcd Corr para' % (Cov_ret_names[i]))
...
mod = SARIMAX(Cov_ret_TS_Co_Inte[1], trend='c', order=order, seasonal_order=seasonal_order,enforce_stationarity=False,enforce_invertibility=False)
res = mod.fit(disp=False)
print(res.summary())
...
eg_pre_ori_merge=pd.merge(pd.DataFrame(pred_ci_mean,columns=['Pred_Data']),pd.DataFrame(Cov_ret_TS_Co_Inte[1]),right_index=True,left_index=True,how='outer')
