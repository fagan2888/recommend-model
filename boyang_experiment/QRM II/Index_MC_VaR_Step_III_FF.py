...
def Gaussian_Copula(data, Index_data_corr_cholesky, Windows_size, suspension_tolerance_filtered_level, Nb_MC, Confidence_level,i):
#    print(data.index[i])
    ...
    log_ret=log_ret.iloc[:Windows_size + i, :]

    
    data_tolerance_filtered = filter_data_by_nan(
        data, suspension_tolerance_filtered_level)
    data_tolerance_filtered = data_tolerance_filtered.fillna(method='ffill')
...
#    data_corr_=data_corr_[data_corr_.index==data.index[-1]]
#    data_corr_=data_corr_.values.reshape(data.shape[1],data.shape[1])
#    
    cholesky_deco_corr_log_ret = Index_data_corr_cholesky[Index_data_corr_cholesky.index==data.index[-1]].iloc[0,0]
#    print(data.index[-1])
...
if __name__ == '__main__':
    ...
    for i in range(Index_data_corr.shape[0]):
        Corr_pred_Matrix_1=pd.DataFrame(Index_data_corr.iloc[i,:].values.reshape(Index_data.shape[1],Index_data.shape[1]))
        Corr_pred_Matrix_1.columns=Index_data.columns
        Corr_pred_Matrix_1.index=Index_data.columns
        Corr_pred_Matrix_final.append(Corr_pred_Matrix_1)
        
    Index_data_corr_cholesky=[]
    for i in range(len(Corr_pred_Matrix_final)):
        try:
            Index_data_corr_cholesky.append(np.linalg.cholesky(Corr_pred_Matrix_final[i]))
        except:
            print('shiiiiiit %s' %(str(Index_data_corr.index[i])))
            Index_data_corr_cholesky.append(np.nan)
            pass
    Index_data_corr_cholesky=pd.DataFrame(Index_data_corr_cholesky,index=Index_data_corr.index,columns=['Cholesky']).fillna(method='ffill')    
...
#    Timeseries_MC = [Gaussian_Copula(data=Index_data.iloc[:Windows_size + i, :], data_corr=Index_data_corr.iloc[:Windows_size + i, :], suspension_tolerance_filtered_level=0.1, Nb_MC=1e+5, Confidence_level=0.99) for i in range(Windows_length)]
    ...
    func = partial(Gaussian_Copula, Index_data, Index_data_corr_cholesky, Windows_size, 0.1, 1e5, 0.99,)
    pool = Pool(32)
    ...
    Timeseries_VaR_Summary.to_csv(r'MC_VaR.csv')
