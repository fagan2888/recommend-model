...
Index_data_Chg_Acc=Index_data_Chg_Acc[Index_data_Chg_Acc.index>'2000-01-01']


Copula=Gaussian_Copula(data=Index_data, suspension_tolerance_filtered_level=0.1, Nb_MC=1e+5, Confidence_level=0.9)
Copula1=Copula.quantile(q=0.05)

Shortfall=Index_data_Chg[Index_data_Chg.iloc[:,0]<=Copula1[0]]
Shortfall.index=Shortfall.index.map(lambda x: pd.Timestamp(x))
Shortfall_shift_1d=Index_data_Chg[Index_data_Chg.index.isin(Shortfall.index+timedelta(days=1))]

A=Shortfall_shift_1d.describe()
