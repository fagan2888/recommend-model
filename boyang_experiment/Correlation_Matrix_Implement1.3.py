...
mod = SARIMAX(Corr_ret_TS_Co_Inte.iloc[:,5], trend='c', order=order, seasonal_order=seasonal_order,enforce_stationarity=False,enforce_invertibility=False)
res = mod.fit(disp=False)
print(res.summary())
...
eg_pre_ori_merge=eg_pre_ori_merge.drop(eg_pre_ori_merge.index[::seasonal_order[3]+1])
...
Corr_vs_fitted_corr=pd.merge(pd.DataFrame(Corr_ret_TS.iloc[:,2]).shift(1),pd.DataFrame(Reverse_Co_Inte),right_index=True,left_index=True,how='inner')
Corr_vs_fitted_corr.plot()
plt.show()

#AQQ=Reverse_Co_Inte.apply(lambda x: x*0+10000 if x >= 1 or x<=-1 else x)
#
#A=pd.DataFrame(np.ones(Corr_ret_TS_Co_Inte.shape[0]))
#
#A.insert(loc=0,column='Corr=2',value=pd.Series(np.ones(Corr_ret_TS_Co_Inte.shape[0])*2))



#########################
ARIMA_calibra_paras_ = pd.read_csv(r"C:\Users\yshlm\Desktop\licaimofang\data\ARIMA_calibra_paras_Summary20.csv",index_col=[0])
...
#A=index_corr_pred(Corr_ret_TS_Co_Inte,ARIMA_calibra_paras_argmin)
...
#    order=ARIMA_calibra_paras_argmin[i][:3]
#    seasonal_order=ARIMA_calibra_paras_argmin[i][-4:]

    order=[int(ARIMA_calibra_paras_argmin[i][j]) for j in range(3)]
    seasonal_order=[int(ARIMA_calibra_paras_argmin[i][j+3]) for j in range(4)]

    order=tuple(order)
    seasonal_order=tuple(seasonal_order)

#    order=tuple((ARIMA_calibra_paras_argmin[i][0],ARIMA_calibra_paras_argmin[i][1],ARIMA_calibra_paras_argmin[i][2]))
#    seasonal_order=tuple((ARIMA_calibra_paras_argmin[i][3],ARIMA_calibra_paras_argmin[i][4],ARIMA_calibra_paras_argmin[i][5],ARIMA_calibra_paras_argmin[i][6]))
...
for i in range(5):       
    Corr_pred_Matrix.insert(loc=i*6,column=str(i),value=pd.Series(np.ones(Corr_pred.shape[0])))
...
Corr_pred_Matrix_.to_csv('Multi_Mkt_Indices_corr_matrix.csv')

'Test whether the matrix is full ranked'
Corr_pred_Matrix_final=[]
for i in range(Corr_pred_Matrix_.shape[0]):
    Corr_pred_Matrix_1=pd.DataFrame(Corr_pred_Matrix_.iloc[0,:].reshape(5,5))
