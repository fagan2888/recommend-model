...
mod = SARIMAX(Corr_ret_TS_Co_Inte.iloc[:,9], trend='c', order=order, seasonal_order=seasonal_order,enforce_stationarity=False,enforce_invertibility=False)
res = mod.fit(disp=False)
print(res.summary())
...
Corr_pred=pd.DataFrame()
for i in range(9,Corr_ret_TS_Co_Inte.shape[1]):
    order= tuple(ARIMA_calibra_paras_argmin[i][:3])
    seasonal_order=tuple(ARIMA_calibra_paras_argmin[i][-4:])
    
    data_ori=Corr_ret_TS_Co_Inte.iloc[:,i]
    mod = SARIMAX(data_ori, trend='c', order=order, seasonal_order=seasonal_order,enforce_stationarity=False,enforce_invertibility=False)
    res = mod.fit(disp=False)
