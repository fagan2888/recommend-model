...
def load_bf_nav(ids, reindex=None, begin_date=None, end_date=None):

    df_nav = {}
    for id_ in ids:
        df_nav[id_] = load_series_2(id_, reindex, begin_date, end_date)
    df_nav = pd.DataFrame(df_nav)

    return df_nav


def load_series_selected(id_, selected_date, reindex=None, begin_date=None, end_date=None):
    # layer = int(id_[-1])
    # id_ = id_[:-2]
    db = database.connection('asset')
    metadata = MetaData(bind=db)
    t1 = Table('barra_stock_factor_selected_factor_nav', metadata, autoload=True)
    ...
    s = select(columns)\
            .where(t1.c.bf_id == id_)\
            .where(t1.c.selected_date == selected_date)

    if begin_date is not None:
        s = s.where(t1.c.trade_date >= begin_date)

    if end_date is not None:
        s = s.where(t1.c.end_date >= end_date)
...
if __name__ == '__main__':
    ...
    df = load_bf_nav(['BF.000001.0', 'BF.000001.1'])
    set_trace()
