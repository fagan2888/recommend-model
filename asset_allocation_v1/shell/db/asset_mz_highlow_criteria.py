...
def save(gid, criteria_id, df):
    ...
    #
    db = database.connection('asset')
    t2 = Table('mz_highlow_criteria', MetaData(bind=db), autoload=True)
    columns = [literal_column(c) for c in (df.index.names + list(df.columns))]
    s = select(columns, (t2.c.mz_highlow_id == gid) & (t2.c.mz_criteria_id == criteria_id))
    df_old = pd.read_sql(s, db, index_col=['mz_highlow_id', 'mz_criteria_id', 'mz_date'], parse_dates=['mz_date'])
    if not df_old.empty:
