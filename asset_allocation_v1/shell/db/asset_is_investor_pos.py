...
def load_fund_pos(gid):
    ...
    index_col = ['is_date', 'is_pool_id', 'is_fund_id']
    
    # if included_investor_id:
    #     columns.insert(0, t1.c.is_investor_id)
    #     index_col.insert(0, 'is_investor_id')

    s = select(columns)

    if gid is not None:
        s = s.where(t1.c.is_investor_id == gid)
    else:
        return None
    # if xtypes is not None:
    #     s = s.where(t1.c.is_type.in_(xtypes))
    
    df = pd.read_sql(s, db, index_col=index_col, parse_dates=['is_date'])

    #
    # 合并来自不同基金池的基金份额
    #
    df_result = df.groupby(level=['is_date', 'is_fund_id']).sum()
