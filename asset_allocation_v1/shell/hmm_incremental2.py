...
class HmmNesc(object):
    ...
    def __init__(self, globalid, start_date=None, end_date=None):
        ...
        self.feature_selected = {
            '120000001':['dpo', 'pct_chg', 'atr', 'pvt'],
            '120000002':['atr', 'pct_chg', 'vstd', 'pvt'],
            '120000013':['pvt', 'pct_chg', 'vstd', 'vma', 'rsi'],
            '120000014':['vstd', 'pct_chg', 'roc', 'wvad'],
            '120000015':['wvad', 'pct_chg', 'sobv', 'cci', 'vma'],
            '120000028':['macd', 'pct_chg', 'atr'],
            '120000029':['roc', 'pct_chg'],
        }
        # 隐形状态数目
        self.state_num = 5
    ...
    @staticmethod
    def feature_select_cv(t_data, features, state_num):
        dates = t_data.index
        feature_selected = set()
        #cv = 0
        for indx in [0, 1]:
            ...
            for date in dates:
                ...
                for feature in features:
                    ...
                    win_ratio = HmmNesc.rating(cv_data, state_num, states, 0)[indx]
                    result[feature] += win_ratio
                #cv += 1
            ...
            sorted_result = sorted(result.iteritems(), key=lambda item: item[1], reverse = True)
            #for key, item in sorted_result:
            #    print key, item/cv
            for ite in range(2):
                feature_selected.add(sorted_result[ite][0])
    ...
    def fs(self):
        self.feature_select_cv(self.ori_data[self.t_start:self.t_end], self.features, \
                self.state_num)
if __name__ == "__main__":
    view_ass = ['120000001', '120000002', '120000013', '120000014', \
                '120000015', '120000029']
    view_ass = ['120000001', '120000002', '120000013', '120000015', '120000029']
    #view_ass = ['120000014']
