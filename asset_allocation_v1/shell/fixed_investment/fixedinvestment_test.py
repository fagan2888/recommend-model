...
if __name__ == '__main__':
    ...
    for i in range(0, len(dates) - period):
        ...
        for j in range(0, period):
            d = dates[i + j]
            total_share += 1.0 / zz500df_m.loc[d]['nav']
            share_ds.append(d)
            shares.append(total_share)
        share_df = pd.DataFrame(shares, index = share_ds, columns = ['nav'])
        share_df = share_df.reindex(tmp_dfr.index).fillna(method='pad')
        tmp_df = (tmp_dfr + 1).cumprod()
        #print share_df
        #print tmp_df
        nav_df = share_df * tmp_df
        df_nav = (nav_df/nav_df.cummax() - 1).min()
        #tmp_df   = tmp_dfr.cumprod()
        #print tmp_df
        #fixinvest_r = total_share * zz500df_m.loc[lastd] / period - 1
        #r           = zz500df_m.loc[lastd] / zz500df_m.loc[startd] - 1
        print '%s,%f' % (startd, df_nav)
