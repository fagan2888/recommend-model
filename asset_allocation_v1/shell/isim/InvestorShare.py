...
class InvestorShare(object):
    
    def __init__(self, investor, policy, debug=True, optfee=True, optt0=False):
    ...
    def process(self, ev):
        ...
        if op == 3:
            ...
            # 处理组合购买
            #
            txn = self.policy.place_buy_order(argv)
            ...
            for order in orders:
                ...
                if in_array(order['op'], [30, 31, 63]):
                    ev2 = (order['nav_date'] + timedelta(hours=18), order['op'], order['order_id'], order['fund_id'], argv)
                else:
                    ev2 = (order['nav_date'] + timedelta(hours=18, minutes=30), order['op'], order['order_id'], order['fund_id'], argv)
                result.append(ev2)
