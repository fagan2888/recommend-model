...
if __name__ == '__main__':
    ...
    fund_df = pd.read_csv('./data/all_stock_fund.csv', index_col = 'SECURITYID', parse_dates = ['OUTSUBBEGDATE'])
    ...
    print share_df
    share_df.to_csv('./data/fund_share.csv')
    ...
    print nav_df
    nav_df.to_csv('./data/fund_nav.csv')
    ...
    dates = pd.date_range('2010-01-01', '2017-02-09')
    fund_size_df = fund_size_df.reindex(dates)
    fund_size_df.fillna(method = 'pad', inplace = True)
    fund_size_df.index.name = 'date'
    print fund_size_df
    fund_size_df.to_csv('./data/fund_size.csv')
