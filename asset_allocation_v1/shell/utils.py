...
def day_2_week2(data_df, trade_dates):
    """
    :usage: 把日数据转化成周数据（只取每周最后一个交易日的数据）
    :param data_df: 输入数据
        type: DataFrame
        format:
                        close_pre   close    open    high     low     volume  \
    ...
    for date in week_dates:
        ...
        tmp_df = tmp_df[tmp_df.index.get_level_values(0) <= high_date]
        for column in columns:
            week_dict[column].append(tmp_df[column][-1])
        low_date = high_date

    week_df = pd.DataFrame(week_dict, index=week_dates)
    # week_df.fillna(method='ffil', inplace=True)
    week_df.to_csv("/home/yaojiahui/recommend_model/asset_allocation_v1/sw_component_week.csv", \
            encoding='utf8')
    return week_df
