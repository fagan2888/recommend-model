...
class RiskManagement(object):
    ...
    def perform(df_nav, df_pos, df_timing):
        ...
        confidence75 = []
        for i in xrange(0, 5):
            df_tmp = DFUtil.nav_drawdown(df_nav[i::5])
            df_c97 = df_tmp.rolling(window=51, min_periods=51).apply(
                lambda x: min(stats.norm.interval(0.97, x.mean(), x.std())))
            df_c97 = df_c97.shift(1)
            df_c75 = df_tmp.rolling(window=51, min_periods=51).apply(
                lambda x: min(stats.norm.interval(0.75, x.mean(), x.std())))
            df_c75 = df_c75.shift(1)
            
            drawdown.append(df_tmp1)
            confidence97.append(df_c97)
            confidence75.append(df_c75)

        df_drawdown = pd.concat(drawdown)
        df_confidence97 = pd.concat(confidence97)
        df_confidence75 = pd.concat(confidence75)

        result = {}
        for asset in df_nav.columns:
            result[asset] = self.control(
                pd.DataFrame({
                    'pos' : df_pos[asset],
                    'drawdown' : df_drawdown[asset],
                    'c97' : df_confidence97[asset],
                    'c75' : df_confidence75[asset],
                    'timing': df_timing[asset]
                })
            )
    ...
    def control(df):
        ...
        for day, row in df.iterrows():
            # 初始化变量
            pos, drawdown, c97, c75, timing = row
            #
            # 检测风控状态是否需要改变
            #
            if status == 0:
