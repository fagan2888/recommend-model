...
def initial_update(gdi,df):
    ...
    today = datetime.now()
    df1 = pd.read_csv('wind_update1.csv').set_index(['date'])
    df2 = pd.read_csv('wind_update2.csv').set_index(['date'])
...
def cal_view(df,para=0.34,win_obs=7,para_obs=7):
    ...
    strength = df['gold_ema'].pct_change().fillna(0).values/para*100
    ####增加利率和汇率同向的经验过滤#################################
    if para_obs==0:
    ...
    data_strength = filter_fuc(data_strength,para*5)#经验参数过滤
...
def cal_gold_view(obs1=7,cot_win=30,ema_win=12):
    ...
    #index_month = trade_date.ATradeDate().month_trade_date(begin_date = '1997-07-01')
    ...
    #df_view = forcast_gold.loc[:,['view_gold','view_cot','view']].fillna(0)
    #df_view['view'] = df_view.loc[:,['view_gold']].values.flatten() * np.where(df_view.view_gold.values*df_view.view_cot.values <= 0,0,1)
    #df_view['view_01'] = np.where(df_view.view > 0,1,np.where(df_view.view==0,0,-1))
...
def view_update(mv,viewid='MC.VW0006'):
    print '########################################'
    #function:"保存MC.VW0006数据"
    #mv = cal_gold_view()
    today = datetime.now()
    df = {}
    df['mc_view_id'] = np.repeat(viewid,len(mv))
    df['mc_date'] = mv.index
    df['mc_inc'] = mv.view_01
    df['created_at'] = np.repeat(today,len(mv))
    df['updated_at'] = np.repeat(today,len(mv))
    df_new = pd.DataFrame(df).set_index(['mc_view_id','mc_date'])
    db = database.connection('asset')
    metadata = MetaData(bind=db)
