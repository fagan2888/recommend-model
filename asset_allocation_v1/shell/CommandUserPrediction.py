...
@click.option('--featurefile', 'optfeaturefile', default=True, help=u'feature file path')
@click.pass_context
def yulai_feat(ctx, optfeaturefile):
    ...
    val_df.label[val_df.label == 1] = 0
    val_df.label[val_df.label == 2] = 1
    ...
    feats = []
    train_df = train_df.reset_index()
    train_df = train_df.set_index(['ts_uid'])
    for uid, group in train_df.groupby(train_df.index):
        num = num + 1
        print uid, num
        group = group.set_index('ts_date')
        group['ts_uid'] = uid
        index = group[group['trade_type3'] > 0.0].index.tolist()
        if len(index) == 0:
            continue
        else:
            for i in range(len(index)):
                #print group.iloc[max(0, i - 7) : i]
                feats.append(group.iloc[max(0, i - 14) : i])
                feats.append(group.iloc[i + 1 : min(len(group), i + 14)])
        #print group.columns[0:100]
        #sys.exit(0)
    ...
    negative_train_df = pd.concat(feats, axis = 0)
    negative_train_df.label = 0
    position_train_df = train_df[train_df.label == 2]
    position_train_df.label = 1


    train_df = pd.concat([negative_train_df, position_train_df], axis = 0)
    train_df = train_df.drop(['trade_type3'], axis = 1)
    test_df = test_df.drop(['trade_type3'], axis = 1)
    val_df = val_df.drop(['trade_type3'], axis = 1)
