...
def fc_rolling(ctx, optid):
    ...
    f1 = open('inner_ret.csv', 'wb')
    f2 = open('outter_ret.csv', 'wb')
    ...
    for date in select_dates:
        ...
        # bf_ids = None
        for i in range(2, 5):
            fc = FactorCluster(assets, i, sdate, edate, fdate, method = method, bf_ids = bf_ids)
            fc.handle()
        ...
        if best_model.silhouette_samples_value < 0.40:
            fc = FactorCluster(assets, 1, sdate, edate, fdate, method = method, bf_ids = bf_ids)
            fc.handle()
        else:
            fc = best_model
...
class FactorCluster(object):
    ...
    def score(self):
        ...
        for layer in self.asset_cluster.keys():
            ...
            # except:
            #     pass
            inner_score.append(cal_score(layer_asset_ret, method = 'inner'))
...
if __name__ == '__main__':
    ...
    # a = pd.DataFrame(np.random.randn(2, 100))
    a = pd.DataFrame(np.array([[0.1, -0.1, 0.2], [0.2, -0.2, 0.3], [0.3, 0.5, 0.8]]))
    start = datetime.datetime.now()
    for i in range(1000):
        adjusted_pearson_affinity(a)
    t1 = datetime.datetime.now()
    for i in range(1000):
        adjusted_pearson_affinity_ori(a)
