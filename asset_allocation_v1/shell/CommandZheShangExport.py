...
def investor_risk_drawdown(ctx, optlist, optstartid, optendid, optstartdate, optenddate, optmonth, output, optexcel):
    ...
    sql = "SELECT is_investor_id, is_date, is_nav FROM `is_investor_nav` WHERE 1  and is_type = 8"
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
...
def investor_month_risk(ctx, optlist, optstartid, optendid, optmonth, output, optexcel):
    ...
    sql = "SELECT is_investor_id , is_date, DATE_FORMAT(is_date, '%%Y-%%m') AS is_month, is_nav FROM `is_investor_nav` WHERE 1 is_type = 8"
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
...
def investor_week_risk(ctx, optlist, optstartid, optendid, optstartdate, optenddate, output, optexcel):
    ...
    sql = "SELECT is_investor_id , is_date, is_nav FROM `is_investor_holding` WHERE 1 is_type = 8"
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
...
def investor_week_return(ctx, optlist, optstartid, optendid, optstartdate, optenddate, output, optexcel):
    ...
    sql = "SELECT is_investor_id , is_date, is_nav FROM `is_investor_holding` WHERE 1 and is_type = 8"
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
