...
def investor_return(ctx, optlist, optstartid, optendid, optmonth, output, optexcel):
    ...
    sql = "SELECT is_investor_id, DATE_FORMAT(is_date, '%%Y-%%m') AS is_month,SUM(is_inc) as is_return FROM `is_investor_nav` WHERE 1 and is_type = 8"
...
def investor_risk_drawdown(ctx, optlist, optstartid, optendid, optstartdate, optenddate, optmonth, output, optexcel):
    ...
    sql = "SELECT is_investor_id, is_date, is_nav FROM `is_investor_nav` WHERE 1  and is_type = 9"
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
...
def investor_month_risk(ctx, optlist, optstartid, optendid, optmonth, output, optexcel):
    ...
    sql = "SELECT is_investor_id , is_date, DATE_FORMAT(is_date, '%%Y-%%m') AS is_month, is_nav FROM `is_investor_nav` WHERE 1 and is_type = 9"
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
...
@click.option('--excel/--no-excel', 'optexcel', default=False, help=u'export excel, default is csv')
@click.pass_context
def investor_week_return(ctx, optlist, optstartid, optendid, optstartdate, optenddate, output, optexcel):
    '''run constant risk model
    '''
    import sys
    # sys.setdefaultencoding() does not exist, here!
    ...
    df_result = pd.read_sql(sql, db,  index_col=['is_investor_id', 'is_date'])
    df_result = df_result.unstack(0)
    df_result = df_result.fillna(method = 'pad').fillna(1.0)
    df_result = df_result.iloc[-1] - df_result.iloc[0]

    if optexcel:
        if output is not None:
            path = output
        else:
            path = datapath('export-week_return.xlsx')
        ...
        if output is not None:
            ...
            path = datapath('export-week_return.csv')

        df_result.to_csv(path)

    print "export week_return to file %s" % (path)
...
@click.option('--excel/--no-excel', 'optexcel', default=False, help=u'export excel, default is csv')
@click.pass_context
def investor_week_absolute_return(ctx, optlist, optstartid, optendid, optstartdate, optenddate, output, optexcel):
    '''run constant risk model
    '''
    import sys
    # sys.setdefaultencoding() does not exist, here!
    ...
    sql = "SELECT is_investor_id, is_date, is_return FROM `is_investor_holding` WHERE 1 "
    if optstartid is not None:
        sql += " AND is_investor_id >= '%s' " % optstartid
    if optendid is not None:
        sql += " AND is_investor_id <= '%s' " % optendid
    ...
    #df_result = df_result.iloc[-1] - df_result.iloc[0]
    df_result = df_result.sum(axis = 0)
