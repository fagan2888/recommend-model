...
def bond_fund_value(start_date, end_date):
    ...
    #  type_sql = "SELECT DISTINCT yt_fund_id FROM yinhe_type WHERE (yt_l2_type IN ('200301', '200501')) AND (yt_begin_date <= '%s' AND (yt_end_date = '0000-00-00' OR yt_end_date >= '%s'))" % (end_date, end_date)
    #
    # 按照成立时间筛选基金
    #
    regtime_sql = "SELECT DISTINCT globalid FROM ra_fund WHERE ra_regtime<='%s' and ra_regtime!='0000-00-00' AND (ra_mask & 0x01) = 0" % (start_date);
