...
def trade_day_index(start_date, end_date=None):
    conn  = MySQLdb.connect(**config.db_base)
    ...
    sql = "SELECT td_date as date FROM trade_dates WHERE td_date BETWEEN '%s' AND '%s' ORDER By td_date ASC" % (start_date, end_date);
...
def trade_day_lookback_index(end_date=None, lookback=126, include_end_date=True):
    if include_end_date:
        condition = "(td_date = '%s')" % (end_date)
