...
def all_trade_dates():
    ...
    sql = "SELECT td_date FROM trade_dates WHERE td_date >= '2002-01-04' AND td_type & 0x02";
...
def trade_dates(start_date, end_date):
    ...
    sql = "SELECT td_date FROM trade_dates WHERE td_date BETWEEN '%s' AND '%s' AND td_type & 0x02" % (start_date, end_date);
...
def index_value(start_date, end_date):
    ...
    #
    #
    sql = "SELECT ra_index_id, ra_index_code, ra_date, ra_nav FROM ra_index_nav, (%s) E WHERE ra_date = E.td_date" % (date_sql)
