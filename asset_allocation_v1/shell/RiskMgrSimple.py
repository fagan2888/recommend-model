...
class RiskMgrSimple(object):
    ...
    def __init__(self, maxdd=-0.075, mindd=-0.03, period=252):
        self.maxdd = maxdd,
        self.mindd = mindd, 
        self.period = period
    ...
    def perform(self, asset, df_input):
        ...
        # action: 0:无风控; 1:无条件空仓; 2:等待择时加仓信号; 3:择时满仓
        #
        # 风控逻辑:
        #
        ...
        for day, row in df.iterrows():
            ...
            # 是否启动风控
            #
            if row['inc2d'] < self.maxdd or row['inc3d'] < self.maxdd:
            ...
            if status == 0:
                ...
                if empty_days >= 5:
                    if row['timing'] == 1.0:
                        empty_days = 0
                        status, position, action = 0, 1, 3 # 择时满仓
                    else:
                        empty_days += 1
                        status, position, action = 1, 0, 2 # 空仓等待择时加仓信号
                else:
                    empty_days += 1
                    status, position, action = 1, 0, 1 # 无条件空仓
