...
if __name__ == '__main__':
    ...
    m2 = pd.read_csv('data/m2.csv', index_col = 0, parse_dates = True)
    m2 = m2.loc['1996':]
    #print m2
    #plt.plot(m2)
    #plt.show()

    mod = SETAR(m2, order=2, delay=2, ar_order=5, thresholds=(14.1,))
    res = mod.fit()

    #print res.summary()
    forecast = res.predict()
    '''
    dates = pd.date_range(start = '2017-10-31', end = '2018-03-31', freq='m')
    for date in dates:
        m2.loc[date] = np.nan
    m2.fillna(method = 'pad', inplace=True)
    print len(m2)
    print len(forecast)
    '''
    m2 = m2.iloc[5:, :]
    m2['forecast'] = forecast
    m2.plot()
    plt.show()
