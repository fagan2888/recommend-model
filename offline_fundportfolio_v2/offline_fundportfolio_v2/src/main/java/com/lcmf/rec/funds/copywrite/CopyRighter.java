...
public class CopyRighter {
	...
	public static String history_bottom_desc(double annual_return, double hs300_return, double portfolio_downdraw,
		...
		if(annual_return > hs300_return && portfolio_downdraw < hs300_downdraw){
			String desc_base = "上图为配置方案与沪深300指数在过去几年的收益表现走势图。通过图表可知，长期来看，当前为您提供的配置方案在过去几年中获取了比沪深300指数高出%.1f%%的平均年化收益，且当遇到行情剧烈波动时，您当前的配置方案更加稳健，较之沪深300指数能够减少%.1f%%的损失。";
			...
			String desc_base = "上图为配置方案与沪深300指数在过去几年的收益表现走势图。通过图表可知，长期来看，当前为您提供的配置方案在过去几年中获取了比沪深300指数高出%.1f%%的平均年化收益。同时，该配置方案属于高风险高回报的组合，短期内可能会出现较大波动，长期持有能更好的规避风险。";
			desc = String.format(desc_base, (annual_return - hs300_return) * 100);
		}else if(annual_return <= hs300_return){
			String desc_base = "上图为配置方案与沪深300指数在过去几年的收益表现走势图。通过图表可知，当前为您提供的配置方案的历史收益较沪深300指数略低，但当遇到行情剧烈波动时，该配置方案更加稳健，较之沪深300指数能够减少%.1f%%的损失，短期内，较适合在震荡行情下投资。";
	...
	public static String liquidity_bottom_desc(){
		//保留两位小数，四舍五入
