...
def beta_index(beta_df, stock_dfr):
    ...
    for d in dates:
        ...
        l             = len(betas)
        low_betas     = betas[0 :(int)(0.5 * l)]
        high_betas    = betas[(int)(0.5 * l) : -1]
        ...
        r = np.mean(low_betas_rs) - np.mean(high_betas_rs)
        print d, r
        rs.append(r)

    df = pd.DataFrame(rs, index = dates, columns = ['beta_r'])
