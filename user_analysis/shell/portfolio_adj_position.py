...
if __name__ == '__main__':
    ...
    for i in range(0, 10):
        sql = 'select ra_date as date, ra_fund_code as code, ra_fund_ratio as ratio from ra_portfolio_pos where ra_portfolio_id = 8009250%d' % i
        df = pd.read_sql(sql, conn, index_col = ['date'], parse_dates = ['date'])
        #df = df[df.index >= '2014-09-25']
        ...
        print i, len(df) / 5.0 ,turnover, len(df)
