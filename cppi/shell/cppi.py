...
def combine_pos(cppi_pos_df, high_risk_pos_df, low_risk_pos_df, high_risk, low_risk):

    cppi_pos_df = result_df[[high_risk, low_risk]]

    low_risk_pos_df = low_risk_pos_df.reindex(cppi_pos_df.index)
    low_risk_pos_df = low_risk_pos_df.fillna(method = 'pad')
    high_risk_pos_df = high_risk_pos_df.reindex(cppi_pos_df.index)
    high_risk_pos_df = high_risk_pos_df.fillna(method = 'pad')

    low_risk_pos_df = pd.DataFrame(np.multiply(low_risk_pos_df.values, cppi_pos_df[[low_risk]].values), index = low_risk_pos_df.index, columns = low_risk_pos_df.columns)
    high_risk_pos_df = pd.DataFrame(np.multiply(high_risk_pos_df.values, cppi_pos_df[[high_risk]].values), index = high_risk_pos_df.index, columns = high_risk_pos_df.columns)

    risk_pos_df = pd.concat([high_risk_pos_df, low_risk_pos_df], axis = 1)
    return risk_pos_df
